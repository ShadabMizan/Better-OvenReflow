...
; Keypad: A=reflow temp, B=reflow time, C=soak temp, D=soak time; E=enter, F=backspace
input_mode:    ds 1      ; 0=none, 1=reflow temp, 2=reflow time, 3=soak temp, 4=soak time
input_buffer:  ds 1      ; value being entered (0-255)
input_digits:  ds 1      ; count of digits (0-3)
...
; Keypad pins from Read_keypad.asm: rows output, columns input
ROW1 EQU P1.2
ROW2 EQU P1.4
ROW3 EQU P1.6
ROW4 EQU P2.0
...
Configure_Keypad_Pins:
	; From Read_keypad.asm: rows output, columns input
...
; Keypad scan - returns C=1 and key in R7 if pressed. Layout: 1,2,3,A; 4,5,6,B; 7,8,9,C; *,0,#,D. E=#=0x0F enter, F=*=0x0E delete.
Keypad_Scan:
	setb COL1
	setb COL2
	setb COL3
...
; A=reflow temp, B=reflow time, C=soak temp, D=soak time; E(#)=enter, F(*)=delete
Process_Keypad_Input:
	lcall Keypad_Scan
	jnc M2_Key_Done
	mov a, input_mode
	jz M2_Check_Mode_Keys
	mov a, R7
	cjne a, #0FH, M2_Check_F
	lcall M2_Enter_Value
	sjmp M2_Key_Done
M2_Check_F:
	cjne a, #0EH, M2_Check_Digit
	lcall M2_Backspace_Digit
	sjmp M2_Key_Done
M2_Check_Digit:
	mov a, R7
	cjne a, #0AH, M2_Maybe_Digit
	sjmp M2_Key_Done
M2_Maybe_Digit:
	jc M2_Add_Digit
	sjmp M2_Key_Done
...
M2_Enter_Value:
	mov a, input_mode
	cjne a, #1, M2_EV_Mode2
	mov a, input_buffer
	mov reflowtmp, a
	sjmp M2_Exit_Input
M2_EV_Mode2:
	cjne a, #2, M2_EV_Mode3
	mov a, input_buffer
	mov reflowtime, a
	sjmp M2_Exit_Input
M2_EV_Mode3:
	cjne a, #3, M2_EV_Mode4
	mov a, input_buffer
	mov soaktmp, a
	sjmp M2_Exit_Input
M2_EV_Mode4:
	cjne a, #4, M2_Exit_Input
	mov a, input_buffer
	mov soaktime, a
M2_Exit_Input:
	mov input_mode, #0
...
Reflow_Temp_Label:  db 'reflow temp  ', 0
Reflow_Time_Label:  db 'reflow time  ', 0
Soak_Temp_Label:    db 'soak temp    ', 0
Soak_Time_Label:    db 'soak time    ', 0
Initial_Message:    db 'Voltmeter test', 0

; Write variable label + entered digits to LCD row 1 (cursor already at 0x80 for Update_LCD_Status)
Update_Input_Display_Only:
    push acc
    push b
    push ar0
    push ar1
    ...
    mov a, input_mode
    cjne a, #1, UID_Label2
    mov dptr, #Reflow_Temp_Label
    sjmp UID_Label_Done
UID_Label2:
    cjne a, #2, UID_Label3
    mov dptr, #Reflow_Time_Label
    sjmp UID_Label_Done
UID_Label3:
    cjne a, #3, UID_Label4
    mov dptr, #Soak_Temp_Label
    sjmp UID_Label_Done
UID_Label4:
    mov dptr, #Soak_Time_Label
UID_Label_Done:
...
; Full update - position to row 1 then write (for keypad handlers)
Update_Input_Display:
    push acc
    mov a, #80h
    lcall ?WriteCommand
    pop acc
    lcall Update_Input_Display_Only
    ret
...
Row1_Done:
    ...
    mov a, #0C0h
    lcall ?WriteCommand
    ; Temp from BCD (temp*1000): bcd+2=0x02, bcd+1=0x30 for 23.0C
    mov a, #'T'
    lcall ?WriteData
    mov a, #' '
    lcall ?WriteData
...
FSM_State1:
    ...
    ; Safety: if temp does not reach 50 C within 60 sec of state 1 start -> error
    mov a, current_time
    ...
    clr c
    subb a, #50
    jnc FSM_State1_Check_Soak   ; current_tmp >= 50 -> OK, continue
